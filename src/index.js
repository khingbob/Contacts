...
ReactDOM.render(<List />, id("listDiv"));
ReactDOM.render(<HiddenListEl />, id("hiddenListDiv"));

ReactDOM.render(<Top />, id("topModule"));
function menuClose() {
  id("settingsMenu").style.transition =
    "top 0.3s ease-in, right  0.3s ease-in, font-size 0.5s ease-in, height 0.5s ease-in, padding 0.5s 0.2s, width 0.6s ease-in-out";
  ...
  setTimeout(function () {
    id("settingsMenu").style.transition =
      "top 0.3s ease-out, right 0.3s ease-out, font-size 0.5s ease-in, height 0.5s ease-in, padding 0.5s 0.2s, width 0.6s ease-in-out";
...
function menuOpen() {
  id("settingsMenu").style.transition =
    "top 0.3s ease-in, right 0.3s ease-in, font-size 0.5s ease-in, height 0.5s ease-in, padding, width 0.6s ease-in-out";
  ...
  setTimeout(function () {
    id("settingsMenu").style.transition =
      "top 0.3s ease-out, right 0.3s ease-out, font-size 0.5s ease-in, height 0.5s ease-in, padding, width 0.6s ease-in-out";
...
id("settings").addEventListener("click", () => {
  menuOpen();
});
id("offTop").addEventListener("click", () => {
  menuClose();
});
id("topContent").addEventListener("click", () => {
  menuClose();
});
id("list").addEventListener("click", () => {
  menuClose();
});
id("searchBar").addEventListener("click", (e) => {
  menuClose();
});
id("searchButton").addEventListener("click", (e) => {
  menuClose();
  append();
});
id("count");
...
function resize() {
  if (id("hiddenDetailsContainer").classList.contains("open")) {
    id("hiddenDetailsContainer").style.marginTop =
      "calc(" + css("visible", "height") + "px" + " + 0.5vh)";
  }
  for (var i = 0; i < clas("countries").length; i++) {
    clas("countries")[i].style.width =
      (css("country0", "height") * 3) / 2 + "px";
  }
